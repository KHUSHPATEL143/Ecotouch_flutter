...
final attendanceReportProvider = FutureProvider.family<List<Map<String, dynamic>>, DateTimeRange>((ref, range) async {
...
class AttendanceReportTab extends ConsumerWidget {
  ...
  Widget _buildExportButton() {
    return ElevatedButton.icon(
      onPressed: () {}, // TODO: Connect to ExportService
  ...
  Widget _buildAttendanceMatrix(BuildContext context, List<Map<String, dynamic>> rawData, DateTimeRange range) {
    ...
    for (var row in rawData) {
      ...
      final dateStr = (row['date'] as String).split('T')[0]; // Format: YYYY-MM-DD
    ...
    final days = List.generate(daysCount, (index) => range.start.add(Duration(days: index)));
        ...
        Expanded(
          child: Scrollbar(
            ...
            child: SingleChildScrollView(
              ...
              child: SizedBox(
                width: 280.0 + (days.length * 60.0), // Fixed width: 200(Name) + 80(Total) + days*60
                child: Column(
                  children: [
                     Container(
                      height: 50,
                      ...
                      child: Row(
                        children: [
                          const SizedBox(width: 200, child: Padding(
                            padding: EdgeInsets.only(left: 24),
                            child: Align(alignment: Alignment.centerLeft, child: Text('WORKER NAME', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold))),
                          )),
                          ...
                          const SizedBox(width: 80, child: Center(child: Text('TOTAL', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold)))),
                        ],
                      ),
                    ),
                    Expanded(
          ...
          child: Row(
            children: [
               Column(
                 ...
                 children: [
                   Text('TOTAL ACTIVE WORKERS', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Theme.of(context).textTheme.bodyMedium?.color?.withOpacity(0.7))),
                   const SizedBox(height: 4),
                   Text('${workerNames.length}', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                 ],
               ),
               ...
               Column(
                 crossAxisAlignment: CrossAxisAlignment.start,
                 children: [
                   Text('AVG ATTENDANCE', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Theme.of(context).textTheme.bodyMedium?.color?.withOpacity(0.7))),
                   const SizedBox(height: 4),
                   const Text('92.4%', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: AppColors.success)),
                 ],
               ),
            ],
          ),
