...
final stockViewTypeProvider = StateProvider<StockViewType>((ref) => StockViewType.rawMaterials);

final rawMaterialStockProvider = FutureProvider.family<List<StockItem>, DateTime>((ref, date) async {
  return await StockCalculationService.getRawMaterialStockItems(date);
});

final productStockProvider = FutureProvider.family<List<StockItem>, DateTime>((ref, date) async {
  return await StockCalculationService.getProductStockItems(date);
});

final rawMaterialStockByBagSizeProvider = FutureProvider.family<List<StockByBagSize>, DateTime>((ref, date) async {
  return await StockCalculationService.calculateRawMaterialStockByBagSize(date);
});

final productStockByBagSizeProvider = FutureProvider.family<List<StockByBagSize>, DateTime>((ref, date) async {
...
class InventoryScreen extends ConsumerWidget {
  ...
  Widget _buildStockTable(BuildContext context, WidgetRef ref, DateTime selectedDate, StockViewType viewType) {
                  ...
                  child: SingleChildScrollView(
                    child: DataTable(
                      ...
                      columns: const [
                        ...
                        DataColumn(label: Text('Total Stock')), // Absolute Quantity
                        DataColumn(label: Text('Status')),
                      ],
                      rows: stockItems.map((item) {
                        return DataRow(
                          onSelectChanged: (_) => _showStockDetails(context, ref, item, selectedDate, viewType),
                          cells: [
                            DataCell(Row(
                              children: [
                                Container(
                                  ...
                                  child: Icon(
                                    viewType == StockViewType.rawMaterials ? Icons.science_outlined : Icons.inventory_2_outlined,
                                    size: 20,
  ...
  void _showStockDetails(BuildContext context, WidgetRef ref, StockItem item, DateTime date, StockViewType viewType) {
    showDialog(
      context: context,
            ...
            builder: (context, ref, _) {
              ...
              return detailsAsync.when(
                data: (allStock) {
                  ...
                  final details = allStock.where((s) => s.materialId == item.id).toList();
                  ...
                  // Sort by bag size
                  details.sort((a, b) => b.bagSize.compareTo(a.bagSize));
                        ...
                        return Padding(
                          ...
                          child: Row(
                            children: [
                              ...
                              const Text(' - '),
                              Text('${d.bagCount} ${d.containerUnit}', style: const TextStyle(fontWeight: FontWeight.w500)),
                              const Spacer(),
                              const Text('='),
                              const Spacer(),
                              Text(
                                '${d.totalWeight.toStringAsFixed(2)} ${d.unit}',
                                style: const TextStyle(fontWeight: FontWeight.bold),
                              ),
                            ],
                          ),
                      ...
                      Padding(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Text(
                              '${totalCalc.toStringAsFixed(2)} ${item.unit}',
