...
class CategoriesManagement extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<CategoriesManagement> createState() => _CategoriesManagementState();
}
...
class _CategoriesManagementState extends ConsumerState<CategoriesManagement> {
  ...
  Widget build(BuildContext context) {
    ...
    final productsAsync = ref.watch(productsByCategoryProvider(_selectedCategory?.id));
                        ...
                        return Card(
                          child: Center(
                            child: Column(
                              ...
                              children: [
                                Icon(Icons.category_outlined, size: 48, color: Theme.of(context).iconTheme.color?.withOpacity(0.5)),
                                const SizedBox(height: 16),
                        ...
                        return Card(
                          child: Center(
                            child: Column(
                              mainAxisAlignment: MainAxisAlignment.center,
                              children: [
                                Icon(Icons.touch_app_outlined, size: 64, color: Theme.of(context).iconTheme.color?.withOpacity(0.3)),
                                const SizedBox(height: 16),
                                Text(
                                    ...
                                    child: DataTable(
                                      ...
                                      rows: products.map((product) {
                                        return DataRow(cells: [
                                          ...
                                          DataCell(Text('${product.initialStock}')),
                                          DataCell(Row(
...
class _ProductDialogState extends ConsumerState<_ProductDialog> with SingleTickerProviderStateMixin {
  final _formKey = GlobalKey<FormState>();
  ...
  void initState() {
    ...
    _unitController = TextEditingController(text: widget.product?.unit ?? 'pieces');
    _initialStockController.text = (widget.product?.initialStock ?? 0).toString();
  ...
  void _addRecipeItem(int rawMaterialId, String name, String unit) {
    // Check if exists
    final exists = _recipeItems.any((item) => item['raw_material_id'] == rawMaterialId);
    if (!exists) {
      setState(() {
        _recipeItems.add({
              ...
              children: [
                TabBar(
                  ...
                  tabs: const [
                    ...
                    Tab(text: 'Recipe (BOM)', icon: Icon(Icons.format_list_bulleted)),
                  ],
                ),
                Expanded(
                  child: TabBarView(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Row(
                                children: [
                                  ...
                                  PopupMenuButton<String>(
                                    ...
                                    itemBuilder: (context) => ['pieces', 'boxes', 'kg', 'tons', 'sets', 'Bag']
                              ...
                              TextFormField(
                                ...
                                validator: (value) => Validators.nonNegativeNumber(value, fieldName: 'Stock'),
                              ),
                            ],
                          ),
                        ),
       ...
       children: [
         ...
         Padding(
           ...
           child: rawMaterialsAsync.when(
             data: (materials) {
               return DropdownButtonFormField<RawMaterial>(
                 decoration: const InputDecoration(
                   labelText: 'Add Raw Material to Recipe',
                 ...
                 onChanged: (material) {
                   if (material != null) {
                     _addRecipeItem(material.id!, material.name, material.unit);
                   }
                 },
               );
             },
               ...
               : ListView.separated(
                   ...
                   itemBuilder: (context, index) {
                     ...
                     return ListTile(
                       ...
                       subtitle: Text('Unit: ${item['raw_material_unit']}'),
                       trailing: Row(
                         mainAxisSize: MainAxisSize.min,
                         children: [
                           SizedBox(
                             width: 100,
                             child: TextFormField(
                               initialValue: item['quantity_ratio'].toString(),
                               decoration: const InputDecoration(
                                 labelText: 'Qty',
                                 isDense: true,
                                 border: OutlineInputBorder(),
                               ...
                               keyboardType: TextInputType.number,
                               onChanged: (val) => _updateRecipeQuantity(index, val),
                             ),
                           ),
                           const SizedBox(width: 8),
                           IconButton(
