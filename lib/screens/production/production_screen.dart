...
final productionListProvider = FutureProvider.family<List<Production>, DateTime>((ref, date) async {
...
class _ProductionScreenState extends ConsumerState<ProductionScreen> {
  ...
  final _unitCountController = TextEditingController(); // For pieces/packs count
  ...
  void _updateRawMaterialQuantities() {
    ...
    for (var item in _productBOM) {
      ...
      if (_rawMaterialQuantityControllers.containsKey(rawMaterialId)) {
        _rawMaterialQuantityControllers[rawMaterialId]!.text = totalRequired.toStringAsFixed(2);
      }
    }
  ...
  Future<void> _handleExport() async {
    ...
    try {
      ...
      if (data.isEmpty) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('No production records found for selected period')),
          );
        }
        return;
      }
      ...
      final rows = data.map((e) {
        final workerNames = e.workerIds?.map((id) => workerMap[id] ?? 'Unknown').join(', ') ?? 'None';
      ...
      final title = 'Production Report (${app_date_utils.DateUtils.formatDate(start)} - ${app_date_utils.DateUtils.formatDate(end)})';
                       ...
                       child: Column(
                         ...
                         children: [
                            Row(
                              children: [
                                Text(
                                  _editingId == null ? 'New Batch Configuration' : 'Edit Batch Configuration',
                           ...
                           // Product Selection
                             ...
                             // Batches & Qty
                              ...
                              TextFormField(
                                ...
                                validator: (value) => Validators.positiveInteger(value, fieldName: 'Batches'),
                              ...
                              if (_selectedProduct?.unit != null && _selectedProduct!.unit!.isNotEmpty)
                                Row(
                                  children: [
                                    Expanded(
                                      child: TextFormField(
                                        ...
                                        decoration: InputDecoration(
                                          labelText: 'Size per ${_selectedProduct!.unit!}', // e.g., Size per Pack
                                          helperText: _convertedUnit != null ? 'Unit size ($_convertedUnit)' : 'Unit size',
                                          border: const OutlineInputBorder(),
                                        ),
                                        keyboardType: TextInputType.number,
                                        onChanged: (_) => _calculateTotalQuantity(),
                                        validator: (value) => Validators.positiveNumber(value, fieldName: 'Unit Size'),
                                      ),
                                    ),
                                    ...
                                    Expanded(
                                      child: TextFormField(
                                        ...
                                        decoration: InputDecoration(
                                          labelText: 'No. of ${_selectedProduct!.unit!}', // e.g., No. of Packs
                                          helperText: 'Count',
                                          border: const OutlineInputBorder(),
                                        ),
                                        keyboardType: TextInputType.number,
                                        onChanged: (_) => _calculateTotalQuantity(),
                                        validator: (value) => Validators.positiveNumber(value, fieldName: 'Count'),
                                      ),
                                    ),
                                  ],
                                )
                                ...
                                TextFormField(
                                  ...
                                  keyboardType: TextInputType.number,
                                  validator: (value) => Validators.positiveNumber(value, fieldName: 'Total Quantity'),
                                ),
                           ...
                           // Workers Assignment
                             ...
                             workersAsync.when(
                               data: (workers) {
                                 if (workers.isEmpty) {
                                  return const Text('No active workers available', style: TextStyle(color: AppColors.textSecondary));
                                 }
                           ...
                           // Raw Material Requirements (Vertical Stack now)
                             Container(
                               ...
                               decoration: BoxDecoration(
                                  color: (Theme.of(context).brightness == Brightness.dark 
                                          ? AppColors.darkSurfaceVariant 
                                          : AppColors.lightSurfaceVariant).withOpacity(0.3),
                                  borderRadius: BorderRadius.circular(AppTheme.borderRadius),
                                  border: Border.all(color: AppColors.border),
                               ),
                               child: Column(
                                 ...
                                 children: [
                                   Row(
                                     children: [
                                      const Icon(Icons.science_outlined, color: AppColors.primaryBlue, size: 20),
                                      const SizedBox(width: 8),
                                      Text(
                                    ...
                                    if (_productBOM.isEmpty)
                                      ...
                                      ListView.separated(
                                        ...
                                        itemBuilder: (context, index) {
                                          ...
                                          final options = _availableStock.where((s) => s.materialId == rawMaterialId).toList();
                                              ...
                                              Row(
                                                ...
                                                children: [
                                                      ...
                                                      children: [
                                                        ...
                                                        if (options.isEmpty)
                                                          ...
                                                          DropdownButtonFormField<double>(
                                                            ...
                                                            onChanged: (value) {
                                                              setState(() {
                                                                _selectedBagSizes[rawMaterialId] = value;
                                                              });
                                                            },
                                                  ...
                                                  SizedBox(
                                                    width: 100,
                                                    child: TextFormField(
                                                      controller: _rawMaterialQuantityControllers[rawMaterialId],
                                                      ...
                                                      validator: (value) => Validators.positiveNumber(value, fieldName: 'Qty'),
                                                    ),
                                                  ),
                                                ],
                                              ),
                             ...
                             SizedBox(
                               ...
                               child: ElevatedButton.icon(
                                 ...
                                 icon: Icon(_editingId == null ? Icons.add_circle_outline : Icons.save),
                                 label: Text(_editingId == null ? 'Create Batch Entry' : 'Update Batch Entry'),
                                ...
                                if (productionList.isEmpty) {
                                  return Center(
                                    child: Column(
                                      ...
                                      children: [
                                        Icon(Icons.assignment_outlined, size: 48, color: Theme.of(context).hintColor.withOpacity(0.3)),
                                        const SizedBox(height: 16),
                                        Text(
                                              ...
                                              Expanded(
                                                child: Column(
                                                  ...
                                                  children: [
                                                    Row(
                                                      children: [
                                                        Text('${production.totalQuantity} ${production.innerUnit ?? production.productUnit ?? 'Units'}', style: const TextStyle(fontWeight: FontWeight.bold)),
                                                        const SizedBox(width: 8),
                                                        Text('in ${production.batches} batches', style: TextStyle(color: Theme.of(context).hintColor, fontSize: 12)),
                                                      ],
                                                    ),
                                                    const SizedBox(height: 4),
                                                    Row(
                                                      children: [
                                                        Icon(Icons.people, size: 14, color: Theme.of(context).hintColor),
  ...
  Future<void> _submitEntry() async {
    ...
    try {
      ...
      double? unitCount;
      if (_selectedProduct?.unit != null && _selectedProduct!.unit!.isNotEmpty) {
         if (_unitSizeController.text.isNotEmpty && _unitCountController.text.isNotEmpty) {
      ...
      if (_productBOM.isNotEmpty) {
        final currentStock = await StockCalculationService.calculateRawMaterialStock(selectedDate);
        
        for (final item in _productBOM) {
          final rawMaterialId = item['raw_material_id'] as int;
          final rawMaterialName = item['raw_material_name'] as String;
          final requiredQty = double.parse(_rawMaterialQuantityControllers[rawMaterialId]!.text);
      ...
      for (final item in _productBOM) {
        final rawMaterialId = item['raw_material_id'] as int;
        final quantityUsed = double.parse(_rawMaterialQuantityControllers[rawMaterialId]!.text);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(_editingId != null ? 'Production batch updated successfully' : 'Production batch recorded successfully'),
  ...
  void _editProduction(Production production) async {
    ...
    try {
      ...
      // Raw Materials Usage
      final rawMaterialUsage = await ProductionRepository.getRawMaterialUsage(production.id!);
      ...
      setState(() {
        ...
        for (final usage in rawMaterialUsage) {
          ...
          if (_rawMaterialQuantityControllers.containsKey(rawMaterialId)) {
            _rawMaterialQuantityControllers[rawMaterialId]!.text = quantityUsed.toString();
          }
  ...
  Future<void> _deleteProduction(Production production) async {
    ...
    if (confirm == true && production.id != null) {
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Production batch deleted'), backgroundColor: AppColors.success),
          );
        }
      } catch (e) {
        if (mounted) {
